...
class CategoryNews {
    ...
    async waitForNewsAPI() {
        return new Promise((resolve, reject) => {
            let attempts = 0;
            const maxAttempts = 100; // 10 seconds timeout for better reliability
            
            const checkNewsAPI = () => {
                attempts++;
                if (typeof NewsAPI !== 'undefined' && typeof window.RSSParser !== 'undefined') {
                    console.log('NewsAPI and dependencies loaded successfully');
                    resolve();
                } else if (attempts >= maxAttempts) {
                    console.error('NewsAPI failed to load after', attempts, 'attempts');
